...
func main() {
	orm, err := postgres.InitializePostgres()
	if err != nil {
		panic(err)
	}
	postgres.DoAutoMigration(orm)

	studentRepo := repository.CreateStudentRepository(orm)
	// courseRepo := repository.CreateCourseRepository(orm)
	// tutorRepo := repository.CreateTutorRepository(orm)
	// groupRepo := repository.CreateGroupRepository(orm)
